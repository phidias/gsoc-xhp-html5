    ...
    Timepicker.prototype = {
        ...
        addTimePicker: function(dp_inst) {
            ...
            if (!this.defaults.timeOnly) {
                ...
                regstr = '\\S{' + this.defaults.dateFormat.length + ',}T' + regstr;
            }
            //--------------------------------------
            var order = this.getFormatPositions();
            var patt = new RegExp(regstr, 'i');
            var treg = currDT.match(patt);
            if (treg) {
                if (order.t !== -1)
        ...
        injectTimePicker: function(dp_inst, tp_inst) {
            ...
            if ($dp.find("div#ui-timepicker-div").length == 0) {
                ...
                if (tp_inst.defaults.scope == "week") {
	                ...
	                $dp.find('.ui-datepicker-calendar tbody tr').mouseleave(function(event) {
...
//	                	$dp.find('.ui-datepicker-calendar tbody a.ui-state-hover').removeClass("ui-state-hover");
        ...
        updateDateTime: function(dp_inst, tp_inst) {
            ...
            this.formattedDate = this.formattedDate.replace("w",$.datepicker.iso8601Week(dt));
            console.log(this.formattedDate);*/
            
            if (this.defaults.alwaysSetTime) {
                this.formattedDateTime = this.formattedDate + 'T' + this.formattedTime;
            }
            else {
